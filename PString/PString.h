...
public:
  ...
  void setBuffer(char *buf, size_t size) {_buf=buf; _cur=buf; _size=size;}
  ...
  int format(char *str, ...);
  int formatP(const char *str, ...);
  int format(const __FlashStringHelper *str, ...);
  
  void printP(const char *str);
  
  inline const char *Pstrstr(char *str) {
    return strstr(_buf, str);
  }
  inline const char *Pstrrchr(char c) {
    return strrchr(_buf, c);  
  }
};
